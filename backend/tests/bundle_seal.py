...
def render(inv: dict[str, str], bundle_name: str) -> str:
    lines = [_HEADER, f"bundle: {bundle_name}", f"files: {len(inv)}"]
...
def parse(text: str) -> tuple[dict[str, str], str | None, bool]:
    """(sealed inventory, recorded seal digest, whether the body still hashes)."""
    inv: dict[str, str] = {}
    body_lines: list[str] = []
    recorded = None
...
def _self_test() -> int:
    ...
    with tempfile.TemporaryDirectory() as td:
        ...
        seal_text = render(inv, "bundle")
        sealed, _rec, intact = parse(seal_text)
        ...
        checks["the seal carries only names, hashes and counts"] = all(
            line.startswith(("#", "bundle:", "files:", "seal-sha256: "))
...
def main() -> int:
    ...
    if args.write:
        ...
        args.write.write_text(render(inv, args.bundle.name), encoding="utf-8",
                              newline="\n")
    ...
    if args.verify:
        if not args.verify.is_file():
            ...
            return 2
        sealed, recorded, intact = parse(args.verify.read_text(encoding="utf-8"))
