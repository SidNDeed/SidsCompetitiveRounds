-- 251: i18n client keys for the Info library (Settings "Stats" -> Info tab
-- wiki batch, Aug 23). 68 NEW keys: ~31 article bodies, article/category
-- titles, and the re-keyed ModeInfoText mode docs (their English was CORRECTED
-- this batch - 1v2 rewards multipliers, FFA host settings/pick/leaver rules,
-- 2v2 ready-timeout wording - so per #289 they are new keys and need fresh
-- translations). The additive half of POST /admin/i18n/sync-keys, written as a
-- migration because this seat's AdminSecret does not verify (403, #406/#421).
-- 4 keys retired by this batch ('Free-for-all for 3-10 player', 'One solo player faces a duo ', 'Stats', 'Two teams of two play a best')
-- are deliberately NOT swept here - orphan rows are inert for the client and
-- the next real sync-keys run (desktop seat) owns retirement + proposal
-- supersede, exactly like migration 248's precedent.
...
-- APPLY WITH (or after) the mod release that ships InfoLibrary.cs - the keys
-- are portal-facing only and harmless earlier, but pointless before it.
...
SELECT v.key_id, v.namespace, v.msgctxt, v.source_hash, v.sensitive, NULL, NULL, NOW()
  FROM (VALUES
('624efda8fd0ebd84', 'client', $k251$<color=#FFD94D><b>HOW A PICK ACTUALLY LANDS</b></color>
...
- <color=#7FD4FF>Babel</color> - 1000 live strings. <color=#7FE87F>1000g</color>$k251$, '43b20143fadbcc200b573c5b37b0768ad48fa1ba', TRUE),
('6aac6cbbebced3a5', 'client', $k251$<color=#FFD94D><b>WHO GETS THE SOLO SEAT</b></color>
...
- If a locked room never fills: a warning at 35 seconds, then an automatic return to the menu at 90. <color=#7FE87F>No penalty.</color>$k251$, '4dcbd19d1118a776e37c240a8993203892053054', TRUE),
('98d8d78a1d5e941d', 'client', $k251$<color=#FFD94D><b>WHOLE-LOBBY GATED - REAL GAMEPLAY CHANGES</b></color>
...
<color=#7FD4FF>Auto-continue</color> - mod rooms auto-confirm the rematch prompt. Room-code games deliberately keep the vanilla prompt: after one side clicks Yes, vanilla starts a 10-second timer that kicks that side to the menu if the other never answers - one-sided auto-Yes kills the player it tries to help.$k251$, '347a0be24a5721872306ab043a41e82eebcc112c', TRUE),
('c64eb8b40ee8fec4', 'client', $k251$About this library$k251$, '4e66bcda284b43d3d5f66c282f86cea9fe749319', FALSE),
('b6d9767052faaed9', 'client', $k251$Achievement guide$k251$, 'eed6829db70d00d75f1575ea1f18feaa88b4b97b', FALSE),
('3e3901c226612420', 'client', $k251$Anti-cheat$k251$, '4c632b2b3d50160e981787806714546a155d2569', FALSE),
('c995f8cdd8ce52e7', 'client', $k251$Betting$k251$, '6239d1b9fc40140857855287b71416de29d03380', FALSE),
('66de64e1b1a3b21c', 'client', $k251$Blocking$k251$, 'd785c0d4b3b9c24878b62f64a7bcf78e9506ab27', FALSE),
('3a55d6d7a2ec803c', 'client', $k251$Body colors in ROUNDS are team colors: the game holds four body skins and picks one by team number. Who gets which color follows from how each mode numbers its teams.
...
On modded screens the color is also your team identity: point announcements call out your color's name (MUSTARD got a point, instead of ORANGE), the round-counter dots and the FFA score strip tint to match, and your in-game nametag tints too - unless a paid nametag color is equipped, which always wins. Identity is resolved from shared data, so everyone with the feature on sees the same names and dots; a player with it off sees vanilla bodies and tints (the custom team NAMES still show).$k251$, '2b8293c4e72cd26a912b45941620fcee37ccdda8', FALSE),
('540aa824d2c28b31', 'client', $k251$Bug fixes the mod ships$k251$, '149285f05807fe0061d475398bb6fbfd0917916b', FALSE),
('910f67b5efb72a02', 'client', $k251$Cosmetics & Identity$k251$, '1f26a904194291df57645321da8a65d06c1b48d2', FALSE),
('c84c43c265080ec2', 'client', $k251$Cosmetics live in the F5 Shop tab and are bought with the gold you earn by playing. This page covers every kind, how each equips, how drops and stock work, and where the art comes from.
...
- A vanilla opponent gets a stats page auto-created from the report. <color=#7FE87F>They keep pure vanilla gameplay, and the only part of the mod they can ever see is nametag styling</color> (see <color=#7FD4FF>What unmodded players see</color>).$k251$, 'f9da8a515f187b04c3b8e3e1499e4fd62bec8b7a', TRUE),
('ffd0bcba80bcba7d', 'client', $k251$Every ranked mode scores you with Glicko-2: a rating, plus a measure of how sure the system is about it. This page explains what the numbers mean, why they move the way they do, when each mode updates, and the full rank ladder.
...
- Leaving mid-game is recorded. A leaver keeps their
  ...
  in which case that game does not rate them.$k251$, 'afb7bfc9b084206a7ffa2cc8b3d901c8516df3d3', TRUE),
('d4e55c6cad6429ea', 'client', $k251$Game Mechanics$k251$, '1fae01eacc86b66836f084c259bcb7a1700793cb', FALSE),
('17359b8ec6399a4a', 'client', $k251$Getting better$k251$, 'ba3b5585c9b14b2966768e5003f9dabd61b2ba22', FALSE),
('aa14ce6adac1c333', 'client', $k251$Getting better at competitive ROUNDS is mechanical, not mystical: blocking discipline, netcode awareness, drafting, and reading the numbers the mod already keeps on you. Every tip below is tied to a real mechanic you can go test.
...
- The Discord bot answers mechanics questions with live data: ask it 'how does blocking work', or ask how much elo you'd gain against a named player and it computes the real Glicko preview for both sides, win probability included (both accounts need linked Discord).$k251$, 'ade6d8a331450ec2260ca05e486f8e48e21d0df4', TRUE),
('c5596757a88ea76b', 'client', $k251$Gold betting runs on live 1v1, 2v2 and FFA games. This page covers who can bet, how the odds are priced, exactly when windows open and close, and what happens to your stake.
...
- Your full bet history across all modes is the bets ledger in the F5 menu. Live bettable games are listed in-game and mirrored in the Discord live-bets channel and its bet commands.$k251$, 'a5a0320e13168f43cecad4cfc1ddee6c594f7202', TRUE),
('77d8d2d50b004dd4', 'client', $k251$Grow$k251$, '7e2299a8cab8d708d1c275e314cf72784ffaa7c9', FALSE),
('614580a23bb3bc8f', 'client', $k251$Grow is the one card whose damage depends on the shooter's FRAME RATE. In vanilla, the same Grow bullet fired by a 60 FPS player hits far harder than one fired at 400 FPS - and the mod normalizes it in competitive play.
...
One honest residual: at very low frame rates a normalized bullet can grow slightly LESS than the target (a few percent stacked; more on a heavy hitch). The error always points down - never toward the one-shot.$k251$, '41a4fddf3fcf4db5dd068dc9cf886d86b18d4384', TRUE),
('b0a7c6d3a3a1f50e', 'client', $k251$Guides to every mechanic: blocking, poison, netcode, modes, ratings, rewards, tournaments, anti-cheat, and what the mod does (and deliberately doesn't) change.$k251$, '563c35f5fd7311e5b35e3c9b4db101e77b832981', FALSE),
('6b9260da4b06a2c5', 'client', $k251$How games are tracked$k251$, '2d2ff271edf97f022c61da788ed38114dbe3f96d', FALSE),
('42ca762d56c495c7', 'client', $k251$How tournaments run$k251$, '79ed67952069104e31b39c3145b76a56f1902994', FALSE),
('b5f34f740a723641', 'client', $k251$Known vanilla bugs$k251$, '9c44d8b9070ee32670d2c711ac5c17177adacca6', FALSE),
('15a0134039dff3c3', 'client', $k251$Live competitive games in every mode can be watched from inside the mod - ranked 1v1, 2v2, 1v2 and FFA (ordinary casual 1v1 rooms aren't listed). You see the real game, live, from a seat that is built so it cannot touch the match.
...
- The only barred group is a lobby's own members: you can never bet on your own lobby.$k251$, '8e761bd4b08d438e4c4386b39a7cfaa00c272edf', TRUE),
('60a0927f3dfe3ebd', 'client', $k251$Modes$k251$, '79f5b225cffab11e22dc40b6f05abef1d33d1561', FALSE),
('06906a07a445cfbd', 'client', $k251$Movement & shield tech$k251$, 'd217db06d210b8c6c176928e820ebe2d83c720dd', FALSE),
('a5b68a6c5bcde4fc', 'client', $k251$Netcode & Photon$k251$, 'c3320119dd20477311675076c815e2caa5e43360', FALSE),
('33e496e0673a0869', 'client', $k251$Nobody in a ROUNDS match is the host. Every game runs through Photon's cloud servers, every player simulates the fight locally, and each shooter is the referee of their own bullets. This page is how all of that fits together.
...
- <color=#8A8A93>A game can feel awful while both FPS and ping graphs read flat - stutter in the replicated stream is invisible to both numbers.</color>$k251$, '731149dda4262f45fd2b9920bd5e2584ed18d14a', FALSE),
('c1b4072db0812e45', 'client', $k251$One solo player faces a duo in a best-of-3 series.
...
- Leaving while locked with no games played cancels the
  ...
  normal match report. The beta has no leave counter.$k251$, '3bb894d2341044a3da1369fb9b1cfb1ba55c742b', TRUE),
('5893137c242888d0', 'client', $k251$Open the Info library$k251$, '24ecf950cbf20b50adeae5290099ca654fb8fc93', FALSE),
('507db85caf0e602a', 'client', $k251$Poison & damage over time$k251$, 'cf45d3edae079b8b38a35974796750918fa489e0', FALSE),
('251cda79e9ae8cec', 'client', $k251$Poison has no damage number of its own. The total a poison bullet deals over time equals that bullet's damage stat at the instant it hit you (Toxic Cloud's longer drip rounds a couple percent above it) - which is why the same green bullet sometimes tickles and sometimes one-shots.
...
- Offline and sandbox are pure vanilla - a single simulation has nothing to desync.$k251$, '040dc80582ef2677d666706843f7c57d1b9b407a', TRUE),
('24fd7e49c61320e1', 'client', $k251$ROUNDS ships with real bugs - some cosmetic, some that decide rounds. The catalogue: symptom, real cause, and whether the mod repairs it. Repair details: (see <color=#7FD4FF>Bug fixes the mod ships</color>).
...
For the full is-this-recorded decision tree, see <color=#7FD4FF>When a game counts</color>.$k251$, '917f11b4b3832aee2c3e82d3c9f3ff95def18f6b', TRUE),
('9568055ad9672251', 'client', $k251$Ranked only works if results can be trusted. The short version: every report is signed and identity-checked, both players' clients gather evidence about each other, detectors flag for human review rather than auto-punishing (one narrow farming pattern is the only exception), and a person makes the final call.
...
- A confirmed verdict doesn't fire an automatic penalty. Enforcement runs through separate, individually audited admin tools: bans and series reversal. A ban cuts live access immediately - sessions revoked, queues blocked, ranked refused - and if you ever match a banned player, your client leaves automatically with a notice.$k251$, 'c801ffbe17db6bee98ea4167dbe677e5af7def51', TRUE),
('5ebd9e4ca23d91dc', 'client', $k251$Ratings & Rewards$k251$, '0d48973d722572c1f7817750bcfdc1c93a212f35', FALSE),
('455b2461eac3e08d', 'client', $k251$Ratings (Glicko-2)$k251$, 'cca4166af39b86d542170871f61b1311ebcee489', FALSE),
('04753f943853fd1f', 'client', $k251$Shop & cosmetics$k251$, '13ec3760f87bc6150209123ec11bdef4b9363e74', FALSE),
('09a7675c5e72711f', 'client', $k251$Sid's Competitive Rounds adds a full competitive layer to ROUNDS: ranked 1v1, 2v2 and free-for-all with their own Glicko-2 ratings (plus a 1v2 beta, recorded but unrated for now), an XP and Gold economy with a cosmetic shop, weekly tournaments, live betting on matches, achievements, and Discord integration. All of it is community-built and community-run.
...
If something looks wrong mid-match, file the report right after that session - the attached log is usually what makes a bug findable.$k251$, 'f530ca4de09a53c8c6516e772d8aff7afabe8644', TRUE),
('5d139a2aa3e9277f', 'client', $k251$Spectating$k251$, '0c6b6d4f70cfb4212b1bf230c43ac7f9db37a841', FALSE),
('3bf441e561ef91aa', 'client', $k251$Start Here$k251$, '477344d232738a1940fb13fed234ad833fbb080f', FALSE),
('d1b8be2788734885', 'client', $k251$Team & body colors$k251$, '4f964480316f94e0572347d15d1effbcf33e228a', FALSE),
('18a1d1922241aa36', 'client', $k251$The Mod & Vanilla$k251$, 'ba1d95f2de6da3db559cea3ba7bdc3d360dd4e35', FALSE),
('c8d198d08b2458b4', 'client', $k251$The bot & check-ins$k251$, '40e583e2c5196cc44e0152a1ec7f0e9003b05c8a', FALSE),
('b28a75d1dad8e07b', 'client', $k251$The mod changes vanilla gameplay only where every player agreed to it. This page explains exactly how it decides, and what is guaranteed when you play with non-modded people.
...
<color=#7FE87F>And your opponent never has to know the mod is there: the only thing a non-modded player can ever see of it is nametag styling</color> (see <color=#7FD4FF>What unmodded players see</color>).$k251$, 'd056314a57971f778cacd0c93848f1b7240058eb', TRUE),
('492328683cba65c5', 'client', $k251$The mod ships 50 achievements. Each unlocks exactly once per account and pays gold on the spot - <color=#7FE87F>100 gold</color> by default, up to <color=#7FE87F>1000</color> for the hardest; every line below shows its payout. Browse them under <color=#7FD4FF>My Stats - Achievements</color> in the F5 menu: each row shows the condition, your unlock date, the percentage of known mod players who have it, and its gold. Click a row to see who has earned it (the first 500, in unlock order).
...
- <color=#7FD4FF>Twins!</color> - both players finish with exactly 5 cards each and identical builds, copy counts included. Both unlock it; nobody needs to win. <color=#7FE87F>500g</color>$k251$, 'e01ce49fac28b9d9fd21bfb3e832d94aece20e29', TRUE),
('304838dece8548b7', 'client', $k251$The rule book for what gets recorded and what gets thrown away: games, series, disconnects, bets. If a result looks missing, the reason is almost always on this page.
...
- A 2v2 game that ends without its series id retries the lookup and defers the send; if that also fails, the game is not recorded.$k251$, 'c56bfd5ef74259ffdcb3e6339e5ae62aabc44604', TRUE),
('2423e49b4ee3ddaf', 'client', $k251$Titles are the short labels that render beside your name across the mod - leaderboard rows, Recent Series, Records, Compare, and every T-chat message you send. You can own any number of them, but only one is active at a time: pick it in the Shop tab with <color=#7FD4FF>Set Active</color>.
...
- Titles do NOT appear on the overhead in-game nametag, and players without the mod never see them - every title surface lives in the mod's UI or on Discord.$k251$, '98eb77f46816e71dab3df085044de8ba26f2542a', TRUE),
('243cd0d8462d92a0', 'client', $k251$Tournaments are 8-16 player double-elimination brackets of best-of-3 series, run end to end by the mod and the bot. There are two kinds: <color=#7FD4FF>Sync</color> (the whole bracket plays out in one sitting) and <color=#7FD4FF>Async</color> (each match has a week to happen). Both are ranked - every game moves your normal 1v1 rating.
...
Shield Charge launches you along your AIM direction on any block press, and the launch works during your own block window. Hitting a player cancels the launch, damages and knocks them back, and grants a bonus block. It also combos with the wall rules above: charge into a wall, get your jumps refreshed, wall-jump out.$k251$, 'ed706f2149025b474c9a6e245bf4a9e6908ecbe6', TRUE),
('5dd73a36dbbf1de3', 'client', $k251$What each vanilla bug fix does and where it applies. Gating rules are in <color=#7FD4FF>Vanilla stays vanilla</color>: crash guards run everywhere, gameplay changes only where everyone is modded and agreed.
...
<color=#7FE87F>Your cosmetics never produce a broken visual, an empty rectangle, or a crash on a non-modded screen.</color> <color=#8A8A93>One early glow implementation did leak a visible rectangle to vanilla clients - it was removed for exactly that reason, and glows have rendered mod-side only ever since.</color>$k251$, '628cdffdd054c7b2adf27040cb7f983e06e20fc0', TRUE),
('1bd91200b53d69fb', 'client', $k251$What unmodded players see$k251$, 'bdc8edc8e6f87a7432574daa3dfd55564ef74fdd', FALSE),
('9e4688ee7855897b', 'client', $k251$When a game counts$k251$, '82787788a58b2381733c8d8cd22405f47bf7231b', FALSE),
('b58acd5e29be1ec7', 'client', $k251$XP, Gold & levels$k251$, '13adb395aafe582e56beeb4950df2212c37ed5c3', TRUE),
('0f58f10cbab027e1', 'client', $k251$Your shield absorbs for exactly 0.3 seconds per press, the absorb decision is made on the SHOOTER's machine, and a pile of cards ride the block event. This page covers all of it.
...
-- Post-check: every inserted key exists and is live. Deliberately NOT a
-- whole-namespace count (248's style): if a real sync-keys run lands before
-- or after this migration the totals move, and a strict equality would turn
-- correct ordering into a failed deploy (#168 rerun-safety class). The
-- id-set check is the actual invariant and is order-independent.
DO $$
...
BEGIN
    ...
    END IF;
    RAISE NOTICE 'post-check OK: all 68 info-library keys live';
END $$;
