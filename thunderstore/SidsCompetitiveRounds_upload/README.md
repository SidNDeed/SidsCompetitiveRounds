...
- **Spectator mode** — **WATCH** a live ranked match from the Leaderboard tab or the 2v2 / 1v2 / FFA live panels: no character, no team, no effect on the players. Four seats per game. You join at a clean round boundary with both decks and health rebuilt exactly (rather than dropping into a half-played round showing the wrong state), then get the full arena — bodies, faces, trails, the crown, live card picks — plus a top bar with names, titles, ratings and scores, and hold **Tab** for the scoreboard. Players always see a *Spectators (N)* line, and can turn spectating off for their own games.
- **Five languages: English, Español, Русский, Українська, Svenska** — the whole mod UI, picked on first launch or in Settings. **Ukrainian and Swedish also translate the base game**: ROUNDS ships neither language, so the mod supplies its own — vanilla menus, prompts, and every card name and description render translated. Community translators propose and review corrections through a web portal, and approved fixes reach players without a mod update.
...
- **In-game ↔ Discord chat bridge** — press **T** anywhere in-game to chat, messages round-trip through the Competitive Rounds Discord with ratings and titles attached, with per-language channels.
- **50 achievements** (100-1000g by difficulty, several unlock exclusive titles) — from card-build challenges (5-0 with Barrage, two Glass Cannons...) to marathon streaks (100+ casual wins in a row) and FFA feats (100 kills in one game), with retroactive grants from your existing match history.
